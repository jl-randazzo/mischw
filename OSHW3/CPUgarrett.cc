...
// TODO: some processes finish before I can writes tocont pid 
void scheduler(int signum)
{
    ...
    if(!new_list.empty())
    {
	tocont = new_list.front();
	processes.push_back(new_list.front());
	if (strcmp(running->name,"idle") != 0) {running->switches = 1;}
	if((tocont->pid = fork()) == 0)
	 {
		char param[sizeof(tocont->name)+3];
		strcpy(param, "./");
		strcat(param,tocont->name);
		assertsyscall(execl(param,tocont->name,NULL), < 0);
    ...
    {
	tostop = running;
	tocont = processes.front();
	if (tostop->pid != tocont->pid) {tostop->switches = tostop->switches + 1;}
	processes.pop_front();
...
void process_done(int signum)
{
	...
	//bool isChild(PCB* i) {return cpid == i->pid;}
        //processes.remove_if(isChild);	
	for (list<PCB*>::iterator iterator = processes.begin(); iterator != processes.end(); ++iterator)
	{
		if ((*iterator)->pid == cpid)
		{
			cout << *iterator;
			delete(*iterator);
    ...
    WRITES("finished a process continuing idle");
    running = idle;
...
int main(int argc, char **argv)
{
    ...
    {
	new_list.push_back(create_newPCB(argv[i])); 
	WRITES("added: ");
	WRITES(argv[i]);
	WRITES(" to the list");
	WRITES("\n");
	WRITES("there are: ");
        WRITEI(new_list.size());
	WRITES(" processes in the new PCB");
	WRITES("\n");
